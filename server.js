...
app.get("/api/appointments", async (req, res) => {
    try {
        ...
        for (const cal of targets) {
           try {
             const r = await calendar.events.list({ calendarId: cal.id, timeMin: new Date().toISOString(), maxResults: 10, singleEvents: true, orderBy: 'startTime' });
...
app.post("/api/vapi/check-slots", async (req, res) => {
  try {
      ...
      broadcast("log", `Checking calendar for ${barber}...`);
      ...
      res.json({ results: [{ toolCallId: toolCall.id, result: JSON.stringify({ status: "success", busy_slots: busySlots, info: "Shop open 09:00-19:00" }) }] });
  } catch (error) {
...
app.post("/api/vapi/book", async (req, res) => {
  try {
      ...
      console.log(`[VAPI] Booking ${clientName} with ${barber}`);
      ...
      await calendar.events.insert({
          calendarId,
          resource: { summary: `${service} - ${clientName}`, description: `Vapi Booking: ${barber}`, start: { dateTime: start.toISOString() }, end: { dateTime: end.toISOString() } }
      });
...
// =================================================================
// 🗣️ VAPI TRANSCRIPT WEBHOOK (Fixed & Logging)
// =================================================================
app.post("/api/vapi/webhook", (req, res) => {
    try {
        const msg = req.body.message;
        
        // LOG EVERY INCOMING MESSAGE TO AZURE CONSOLE
        // This way we know if Vapi is hitting the server at all
        if (msg) {
             // Uncomment to see EVERYTHING (noisy): console.log("Vapi Msg Type:", msg.type);
        }
